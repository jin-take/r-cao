...
import { agentName } from "@/data/mvp";

export default function AuditPage() {
  const { auditLogs, audits } = useMvp();
  const alerts = auditLogs.filter((item) => item.policyResult !== "ALLOW");
  return <section className="shell">
    ...
    <div className="audit-summary"><div><strong>{auditLogs.length}</strong><span>recorded events</span></div><div><strong>{alerts.length}</strong><span>alerts / approvals</span></div><div><strong>{audits.length}</strong><span>task audits</span></div><div><strong>0</strong><span>secrets stored</span></div></div>
    ...
    <p className="notice">Audit FAILはOwner Reviewへ進めません。認証Token、秘密鍵、seed phrase、API secretなどの秘密情報はAudit Logへ保存しません。</p>
  </section>;
}
