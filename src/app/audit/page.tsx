...
export default function AuditPage() {
  ...
  const approvals = auditLogs.filter(isOwnerApprovalPending);
  return <section className="shell">
    ...
    <div className="audit-summary"><div><strong>{auditLogs.length}</strong><span>recorded events</span></div><div><strong>{alerts.length}</strong><span>blocking alerts</span></div><div><strong>{approvals.length}</strong><span>owner approvals pending</span></div><div><strong>{audits.length}</strong><span>task audits</span></div></div>
